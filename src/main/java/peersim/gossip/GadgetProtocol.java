...
public class GadgetProtocol implements CDProtocol {
	...
	public void nextCycle(Node node, int pid) {
		...
		if(pn.converged == false){
		    ...
		    // Gossip the loss vectors and update both models.
		    pushLossVectorSquaredLoss(pn, peer, start_index, end_index);
		    //pn = nodeList.get(0);
			//peer = nodeList.get(1);
	        ...
	        if ((iter%10) == 0) {
	        	...
	        	for (int i = 0; i <  Network.size(); i++) {
					PegasosNode tempnode = (PegasosNode) Network.get(i);
					List<Double> train_res = computeLoss(tempnode,  "train");
					train_loss += train_res.get(0);
					train_accuracy += train_res.get(1);
					
				}
	        	
	        	//List<Double> train_res = computeLoss(pn, "train");
	        	//train_loss = train_res.get(0);
		        train_loss = train_loss/Network.size();
		        train_accuracy = train_accuracy/Network.size();
		        
		        System.out.println("Train loss after iteration " + iter + " at node " + pn.getID() + " is:" + train_loss);
		        System.out.println("Train accuracy after iteration " + iter + " on Node " + pn.getID() + " is:" + train_accuracy);
				
		        for (int i = 0; i <  Network.size(); i++) {
					PegasosNode tempnode = (PegasosNode) Network.get(i);
					List<Double> test_res = computeLoss(tempnode, "test");
					test_loss += test_res.get(0);
					test_accuracy += test_res.get(1);
				}
				
				test_loss = test_loss/Network.size();
		        test_accuracy = test_accuracy/Network.size();
		        
				
				System.out.println("Test loss after iteration " + iter + " is:" + test_loss);
				System.out.println("Test accuracy after iteration " + iter + " on Node " + pn.getID() + " is:" + test_accuracy);
